...
export function streamAtMacroIdentifier(ts: TokenStream): boolean {
  if (ts.eof()) return false
  let tok = ts.peek()
  return tok.type === TokenType.Macro || tok.type === TokenType.MacroWithSpace
}
export function streamAtMacroWithSpace(ts: TokenStream): boolean {
  if (ts.eof()) return false
  let tok = ts.peek()
  return tok.type === TokenType.MacroWithSpace
}
export function streamAtMacroWithoutSpace(ts: TokenStream): boolean {
  if (ts.eof()) return false
  let tok = ts.peek()
