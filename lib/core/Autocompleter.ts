...
export class Autocompleter {
  ...
  constructor(public sessionModel: SessionModel, public jumper: JumpController, public controller: Controller) {
  ...
  getSuggestionsRegularRequest(options: AutoCompleteRequestOptions): (Promise<any[]> | any[]) {
    let that = this
    return new Promise<any[]>(async (resolveCb, rejectCb) => {
      ...
      if (!prefix) { resolveCb([]); return }
      if (prefix.trim() === "") { resolveCb([]); return }
      ...
      if (prefix === ".") {
        ...
        let identNode: IdentifierNode = identifiers.getIdentifierForPoint(point)
        if (identNode === null) { resolveCb([]); return }
        let resolve: Resolve = identifiers.map.get(identNode)
        if (!(resolve instanceof ModuleResolve)) { resolveCb([]); return }
        ...
        if (dotCol > 0 && rowText[dotCol] === ".") {
          ...
          if (prefix.length === 1) {
            ...
            await that.controller.reparseFileAsync(path, editor.getText())
            identifiers = getFromHash(parseSet.identifiers, path)
            fileScopes = getFromHash(parseSet.scopes, path)
            fileLevelNode = getFromHash(parseSet.fileLevelNodes, path)
          }
          let identNode: IdentifierNode = identifiers.getIdentifierForPoint(point)
          if (identNode === null) { resolveCb([]); return }
          let resolve: Resolve = identifiers.map.get(identNode)
